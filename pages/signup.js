            ...
            {user ? gotohome() : 
                <div className="w-screen h-screen flex justify-center items-center">
                    <div className="w-full h-full px-10 flex flex-col justify-evenly text-white bg-gray-850 md:max-w-2xl md:h-auto md:py-5 md:items-center md:rounded-lg">
                        ...
                        <h1 className="font-bold text-4xl mt-5">Cadastro</h1>
                        <form className="my-5" onSubmit={registeruser}>
                            ...
                            <input disabled={disable} type="submit" value="Cadastrar" className="w-full py-2 bg-purple-550 text-center text-xl font-semibold rounded-lg cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"/>
                        </form>
                        <div>
                            {registershowerror? registershowerror : null}
                            {errorMessage ? <><p className='text-center text-red-700 font-bold'>{errorMessage}</p></> : null}
                        </div>
                        <Link href="/login"><a className="font-semibold underline text-center outline-none focus:text-purple-550"><p>Login</p></a></Link>
                        </div>
                    </div>
                </div>
            }
